...
export async function GET(req: NextRequest) {
  ...
  const [profileRes, holdingsRes] = await Promise.all([
    supabaseAdmin.from('user_profiles').select('cash').eq('id', auth.userId).single(),
    supabaseAdmin
      .from('holdings')
  ...
  const priceResults = await Promise.allSettled(holdingRows.map((h) => getStockPrice(h.code)))

  const holdings = holdingRows.map((h, i) => {
    const settled = priceResults[i]
    const currentPrice =
      settled.status === 'fulfilled' ? settled.value.currentPrice : h.average_price
    return {
      code: h.code,
  ...
  await supabaseAdmin
    .from('asset_history')
    .upsert(
      { user_id: auth.userId, snapshot_date: today, total_value: totalAssets },
  ...
  const dailyReturnRate = prevValue > 0 ? (dailyReturnAmt / prevValue) * 100 : 0
